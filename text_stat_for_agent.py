...
def text_stat_for_agent(region):
    import sqlite3
    import pandas as pd
    import ast
    ...
    field_texts = []
    for i in ['Саноат', 'Қурилиш', 'Хизматлар', 'Савдо']:
        df = df_region[df_region['Фаолоият соҳангиз:'] == i]
        df = df[['Корхона (ташкилот) номи:', 'Корхона СТИР рақами (ИНН):']]
        df.index = range(1, len(df.index)+1)
        df.reset_index(inplace=True)
        df = df.astype(str)
        df['row_text'] = df['index']+ '. ' + df['Корхона (ташкилот) номи:'] + ', ' + df['Корхона СТИР рақами (ИНН):'] + '\n'
        field_text = i + ':\n\n' + df.row_text.sum()[:-1]
